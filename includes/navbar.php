...
<nav class="backdrop-blur-md bg-white/10 border border-white/20 shadow-lg px-4 py-3 flex justify-between items-center w-[90%] md:w-[70%] lg:w-[55%] fixed top-4 left-1/2 -translate-x-1/2 z-50 rounded-2xl text-white">
  ...
  <div class="hidden sm:flex items-center gap-2">
    <?php if ((isset($_SESSION["username"])) && ($_SESSION["isAdmin"] == 1)): ?>
        ...
        <a href="<?= BASE_URL ?>includes/logout.inc.php" class="text-sm text-red-400 hover:text-red-300 transition">Logout</a>
    <?php elseif (isset($_SESSION["username"])): ?>
        <a href="<?= BASE_URL ?>index.php" class="mr-2 text-sm hover:text-white text-white/70 transition">Shoot Plan</a>
        <a href="<?= BASE_URL ?>includes/dashboard.php" class="mr-2 text-sm hover:text-white text-white/70 transition">Dashboard</a>
        <a href="<?= BASE_URL ?>includes/logout.inc.php" class="text-sm text-red-400 hover:text-red-300 transition">Logout</a>
    <?php else: ?>
        <a href="<?= BASE_URL ?>login.php" class="mr-2 text-sm hover:text-white text-white/70 transition">Login</a>
        <a href="<?= BASE_URL ?>signup.php" class="text-sm hover:text-white text-white/70 transition">Register</a>
    <?php endif; ?>
  </div>
  ...
  <nav class="flex flex-col gap-1 px-4 py-5 flex-1">
    ...
    <?php if ((isset($_SESSION["username"])) && ($_SESSION["isAdmin"] == 1)): ?>
      ...
      <div class="mt-auto pt-4 border-t border-white/10">
        <a href="<?= BASE_URL ?>includes/logout.inc.php"
